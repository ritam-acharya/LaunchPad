...
export default function Swap() {
    ...
    async function performSwap() {
        if(!publicKey) {
            alert("Please connect your wallet to perform the swap.");
            return;
        }
        ...
        try{
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/getPoolAddress/${fromAddress}/${toAddress}`);
            if(!response.data.success) {
                alert("No pool found for the given token pair. Please try again.");
                setLoading(false);
                return;
            }
            const poolAddress = response.data.message.poolAddress;
            const result = await swap(new PublicKey(poolAddress), new PublicKey(fromAddress!), new PublicKey(toAddress!), fromAmount!);
            if (!result) {
                alert("Failed to create pool. Please try again.");
                setLoading(false);
                return;
            }
            const { baseVal, quoteVal } = result;
            const reqBody = {
                baseAmount: baseVal,
                quoteAmount: quoteVal,
            }
            
            const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/v1/updatePool/${poolAddress}`, reqBody);
            ...
            if (res.data.success) {
                ...
                if (toAmountRef.current) {
                    toAmountRef.current.value = "0";
                }
            }else {
                alert("Swap successfull. Failed to update server.");
                setLoading(false);
            }
        }catch {
    ...
    return <div className="px-4 md:px-8 lg:px-12 pt-24 pb-10 h-auto min-h-screen w-full bg-neutral-950 text-white flex flex-col">
        <h2 className="text-[24px] md:text-[28px] lg:text-[32px] font-medium px-2">Swap Tokens</h2>
            ...
            <div className="h-auto w-full max-w-[650px] bg-[#1C243E] rounded-lg px-2 md:px-4 lg:px-6 py-10 ">
                <div className=" text-white flex flex-col items-start justify-between h-auto w-full gap-10 ">
                    ...
                    className="bg-[#32C0D1] text-black h-auto w-full rounded-lg px-4 py-4 flex items-center justify-center cursor-pointer font-medium text-[14px] md:text-[16px] lg:text-[18px] leading-[14px] md:leading-[16px] lg:leading-[18px] tracking-tight  ">
                        {
                            loading ? "Loading..." : publicKey ? "Swap" : "Connect Wallet to Swap"
                        }
                    </div>
                </div>
            </div>
